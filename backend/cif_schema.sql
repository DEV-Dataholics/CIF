...
-- ── 11. Cobranza (Cuentas por Cobrar) ───────────────────────
DROP TABLE IF EXISTS `cobranza_facturas`;
CREATE TABLE `cobranza_facturas` (
  ...
  `created_at`       DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at`       DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY `uq_cobranza_folio` (`folio`),
  UNIQUE KEY `uq_cobranza_uuid` (`uuid`),
  INDEX `idx_cobranza_estatus` (`estatus`),
  INDEX `idx_cobranza_fecha_emision` (`fecha_emision`),
  INDEX `idx_cobranza_fecha_vencimiento` (`fecha_vencimiento`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
